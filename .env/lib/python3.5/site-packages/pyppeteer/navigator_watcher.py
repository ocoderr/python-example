...
class NavigatorWatcher():
    ...
    def __init__(self, frameManager: FrameManager, frame: Frame, timeout: int, options: Dict=None, **kwargs: Any) -> None:
        ...
        self._eventListeners = [helper.addEventListener(self._frameManeger, FrameManager.Events.LifecycleEvent, self._checkLifecycleComplete), helper.addEventListener(
            self._frameManeger, FrameManager.Events.FrameDetached, self._checkLifecycleComplete)]
        loop = asyncio.get_event_loop()
        self._lifecycleCompletePromise = loop.create_future()
        self._navigationPromise = asyncio.ensure_future(asyncio.wait(
            [self._lifecycleCompletePromise, self._createTimeoutPromise()], return_when=concurrent.futures.FIRST_COMPLETED))
    ...
    def _validate_options(self, options: Dict) -> None:
        ...
        if (options.get('waitUntil') == 'networkidle'):
            raise ValueError(
                '`networkidle` option is no logner supported.Use `networkidle2` instead.')
        ...
        for value in waitUntil:
            ...
            if (protocolEvent is None):
                raise ValueError(
                    ''.join(['Unknown value for options.waitUntil: ', '{}'.format(value)]))
    ...
    def _createTimeoutPromise(self) -> Awaitable[None]:
        ...
        if self._timeout:
            errorMessage = ''.join(
                ['Navigation Timeout Exceeded: ', '{}'.format(self._timeout), ' ms exceeded.'])
