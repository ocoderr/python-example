...
class Browser(EventEmitter):
    ...
    @staticmethod
    async def create(connection: Connection, options: dict=None, process: Optional[Popen]=None, closeCallback: Callable[([], Awaitable[None])]=None, **kwargs: Any) -> 'Browser':
    ...
    async def newPage(self) -> Page:
        ...
        targetId = (await self._connection.send('Target.createTarget', {
            'url': 'about:blank',
        })).get('targetId')
...
class Target(object):
    ...
    def __init__(self, browser: Browser, targetInfo: Dict) -> None:
        ...
        self._isInitialized = ((self._targetInfo['type'] != 'page') or (
    ...
    async def page(self) -> Optional[Page]:
        ...
        if ((self._targetInfo['type'] == 'page') and (self._page is None)):
            ...
            new_page = (await Page.create(client, self, self._browser._ignoreHTTPSErrors, self._browser._appMode, self._browser._screenshotTaskQueue))
            self._page = new_page
            return new_page
        return self._page
