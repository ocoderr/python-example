...
class FrameManager(EventEmitter):
    'FrameManager class.'
    Events = SimpleNamespace(FrameAttached='frameattached', FrameNavigated='framenavigated',
    ...
    def _onExecutionContextCreated(self, contextPayload: Dict) -> None:
        context = ExecutionContext(
            self._client, contextPayload, (lambda obj: self.createJSHandle(contextPayload['id'], obj)))
...
class Frame(object):
    ...
    def __init__(self, client: CDPSession, page: Any, parentFrame: Optional['Frame'], frameId: str) -> None:
        self._client = client
        self._page = page
    ...
    async def querySelectorAllEval(self, selector: str, pageFunction: str, *args: Any) -> Optional[Dict]:
        ...
        if (context is None):
            raise ElementHandleError('ExecutionContext is None.')
        arrayHandle = (await context.evaluateHandle('selector => Array.from(document.querySelectorAll(selector))', selector))
    ...
    def waitFor(self, selectorOrFunctionOrTimeout: Union[(str, int, float)], options: dict=None, *args: Any, **kwargs: Any) -> Union[(Awaitable, 'WaitTask')]:
        ...
        if isinstance(selectorOrFunctionOrTimeout, (int, float)):
            fut = asyncio.ensure_future(asyncio.sleep(
                (selectorOrFunctionOrTimeout / 1000)))
            return fut
    ...
    def waitForSelector(self, selector: str, options: dict=None, **kwargs: Any) -> 'WaitTask':
        ...
        return self.waitForFunction(predicate, options, selector, waitForVisible, waitForHidden)
...
class WaitTask(object):
    ...
    def __init__(self, frame: Frame, predicateBody: str, polling: Union[(str, int)], timeout: float, *args: Any) -> None:
        ...
        async def timer(timeout: Union[(int, float)]) -> None:
            ...
            self.terminate(TimeoutError(
                ''.join(['Waiting failed: timeout ', '{}'.format(timeout), 'ms exceeds.'])))
